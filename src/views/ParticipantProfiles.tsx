  ...
  return (
    ...
    >
      <div className="flex items-baseline justify-between mb-1">
        <span className="font-semibold text-grey-5">{p.id}</span>
        <span className="text-sm text-grey-3">{evidenceCount} rows</span>
      </div>
      <p className="text-sm text-grey-4 mb-1">{p.Persona}</p>
      <p className="text-xs text-grey-3">{p.Device}</p>
      {isEcho && <p className="text-xs text-accent mt-1">EchoVision by AGIGA</p>}
    </Link>
  )
}
...
function ProfileDetail({ pid }: { pid: string }) {
  ...
  if (!p) {
    return <p className="text-grey-4">Participant {pid} not found.</p>
  }
  ...
  return (
    <div>
      <Link to="/participants" className="text-sm text-accent hover:underline mb-4 inline-block min-h-[2.75rem] flex items-center">
        ← All participants
      </Link>
      <h1 className="text-xl font-semibold text-grey-5 mb-1">{pid}</h1>
      <p className="text-sm text-grey-4 mb-4">{p.Persona}</p>
      {isP011(pid) && (
      ...
      <h2 className="text-lg font-semibold text-grey-5 mb-2">Attributes</h2>
      <dl className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-2 mb-8">
        {attrs.map(([key, val]) => (
          <div key={key} className="border-b border-grey-1 pb-1">
            <dt className="text-xs text-grey-3 font-medium">{key}</dt>
            <dd className="text-sm text-grey-5">{val != null ? String(val) : '—'}</dd>
          </div>
        ))}
      </dl>

      <h2 className="text-lg font-semibold text-grey-5 mb-2">
        Evidence ({rows.length} rows)
      </h2>
      <div className="overflow-x-auto max-h-[60vh] overflow-y-auto border border-grey-1 rounded">
        <table className="w-full text-sm border-collapse">
          <thead className="sticky top-0 bg-white">
            <tr>
              <th className="text-left px-2 py-2 border-b border-grey-2 font-medium text-grey-4" scope="col">Line</th>
              ...
              <th className="text-left px-2 py-2 border-b border-grey-2 font-medium text-grey-4" scope="col">Quote</th>
            </tr>
          </thead>
          <tbody>
            {rows.map((r: EvidenceRow, i: number) => (
              <tr key={i} className="hover:bg-grey-0">
                <td className="px-2 py-1.5 border-b border-grey-1 font-mono text-xs">{r.Line}</td>
                <td className="px-2 py-1.5 border-b border-grey-1 font-mono text-xs">{r.Code}</td>
                <td className="px-2 py-1.5 border-b border-grey-1">{r.Theme}</td>
                <td className="px-2 py-1.5 border-b border-grey-1">
                  <blockquote className="not-italic">
                    {r.Quote}
                    <cite className="text-xs text-grey-3 ml-1 not-italic">{r.Who} {r.Line}</cite>
                  </blockquote>
                </td>
              </tr>
            ))}
...
export function ParticipantProfiles() {
  ...
  return (
    <section aria-labelledby="participants-heading">
      <h1 id="participants-heading" className="text-xl font-semibold text-grey-5 mb-1">
